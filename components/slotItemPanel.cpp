...
void SlotItemPanel::AfterEdit(wxCommandEvent& event){
	...
	tmp = wxAtoi(this->mItem_cherry_tc->GetValue()) +
		  ...
		  wxAtoi(this->mItem_freecoin_tc->GetValue()) +
		  wxAtoi(this->mItem_roulette_tc->GetValue());
	...
	if(tmp > 10000){
	  ...
	  wxMessageDialog *dial = new wxMessageDialog(NULL,L"Slot Item Sum Over 10000",L"Error", wxOK | wxICON_ERROR);
	  dial->ShowModal();
	}else if(tmp < 10000){
	  this->mTotalValue_tc->SetForegroundColour(wxColour(255,0,0));
	}else{
